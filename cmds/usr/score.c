...
#ifdef LONELY_IMPROVED
	line += sprintf( HIW "【战斗攻击力】" NOR " %10s" YEL " ┃\n" NOR ,
                count_add(attack_points, 1));
#else
...
#ifdef LONELY_IMPROVED
	line += sprintf( HIW "【战斗防御力】" NOR " %10s" YEL " ┃\n" NOR ,
                count_add(count_add(dodge_points, (weapon? parry_points : count_div(parry_points, 10))), 1));
#else                                    
...
#ifdef LONELY_IMPROVED
        line += sprintf( YEL "┃" HIY "【经验】" HIW "%-10s" HIY "【潜能】" NOR "%s%-10s" NOR YEL "┃\n" NOR ,count_add(my["combat_exp"], 0),count_ge(ob->query("potential"), ob->query_potential_limit())?HIM:HIW,count_sub(ob->query("potential"), ob->query("learned_points")) );
#else
...
#ifdef LONELY_IMPROVED
	line += sprintf( HIY "【%s】" HIW "%-10d" HIY "【体会】" HIW "%s%-10d" NOR YEL "┃\n" NOR ,(ob->query("shen")>=0)?"正气":"邪气",ob->query("shen")?ob->query("shen"):0,count_lt(my["combat_exp"], 100000) ? HIR :        
#else
        ...
        line += sprintf( YEL "┃" HIY "【功绩】" HIW "%-10d" HIY "【灵惠】" NOR HIW "%-10d" NOR YEL "┃\n" NOR , ob->query("family/gongji") ,my["magic_points"] - my["magic_learned"] );
        /*
        if (mapp(my["couple"]))
        {
        ...
        */
        ...
        line += sprintf( YEL "┃                                                                ┃\n" NOR );
        line += sprintf( YEL "┃" NOR HIY " 武学宗师： %s" NOR HIY "    任督二脉： %s" NOR HIY "   元婴出世： %s" NOR HIY "    天魔解体：%s " NOR YEL "┃\n" NOR,
                ultrap(ob) ? "○" : HIC "×", ob->query("breakup") ? "○" : HIC "×", ob->query("animaout") ? "○" : HIC "×", ob->query("jieti") ? "○" : HIC "×" );
