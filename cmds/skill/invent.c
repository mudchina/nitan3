...
int main(object me, string arg)
{
        ...
        me->set_temp("invent/mut_level", mut_level);    
...
void get_z_c_action(string yn, object ob)
{
       ...
       // 自创武工的威力由武修、人物等级、智商决定
        s_skill["attack"] = ob->query("level") / 15 + ob->query("int") + 
                            sizeof(ob->query_temp("invent/mut_skills")) * 5 + 
                            ob->query_temp("invent/mut_level") / 10 +                            
                            ob->query_skill("martial-cognize", 1) / 20 - 100;
        if (s_skill["attack"] > 260) s_skill["attack"] = 260;
        s_skill["damage"] = ob->query("level") / 15 + ob->query("str") + 
                            sizeof(ob->query_temp("invent/mut_skills")) * 5 +
                            ob->query_temp("invent/mut_level") / 10 + 
                            ob->query_skill("martial-cognize", 1) / 20 - 100;
        if (s_skill["damage"] > 260) s_skill["damage"] = 260;
        s_skill["force"]  = ob->query("level") / 15 + ob->query("con") + 
                            sizeof(ob->query_temp("invent/mut_skills")) * 5 + 
                            ob->query_temp("invent/mut_level") / 10 +    
                            ob->query_skill("martial-cognize", 1) / 20 - 100;
        if (s_skill["force"] > 260) s_skill["force"] = 260;
        s_skill["dodge"]  = ob->query("level") / 15 + ob->query("dex") + 
                            sizeof(ob->query_temp("invent/mut_skills")) * 5 + 
                            ob->query_temp("invent/mut_level") / 10 + 
                            ob->query_skill("martial-cognize", 1) / 20 - 100;
        if (s_skill["dodge"] > 260) s_skill["dodge"] = 260;
        s_skill["parry"]  = ob->query("level") / 15 + ob->query("con") + 
                            sizeof(ob->query_temp("invent/mut_skills")) * 5 +
                            ob->query_temp("invent/mut_level") / 10 + 
                            ob->query_skill("martial-cognize", 1) / 20 - 100;
