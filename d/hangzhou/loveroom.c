...
void create()
{
        ...
        set("long", @LONG
楼外楼的相约轩，四面通窗。湖风拂面，淡淡生暖。远望苏堤一线，映波，
锁澜，望山，压堤，东浦，跨虹六桥断续连沿，烟柳笼纱，流莺娇啼。近处白堤
横亘湖上，堤上杨柳覆地，芳草寒茵如一段碧绸飘然湖面。室内一张精巧的小桌。
桌上一纸精致的菜单(menu)。
LONG );
        set("exits", ([ /* sizeof() == 1 */
            "down" : "/d/hangzhou/jiulou",
        ]));
        set("sleep_room",1);
        set("item_desc", ([
            "menu": @TEXT
...
int do_open(string arg)
{
        ...
        if(arg!="door") return notify_fail("你要干什么？\n");
        room=this_object();
        room->set("exits/down","/d/hangzhou/jiulou");
        (int)me->query_temp("plugin");
