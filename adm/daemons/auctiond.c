...
void heart_beat()
{
        ...
        for (i = 0; i < sizeof(id); i++)
        {
                ...
                {
                        map_delete(auction_info, id[i]);
                        message_auction(sprintf("由于%s(%s)的拍卖品突然失踪，此次拍卖被取消。", me->name(), id[i])); 
                        continue;
                }
                        ...
                        if (auction_info[id[i]]["state"] >= 3)
                        {
                                ...
                                else
                                {
                                        ...
                                        case 2:
                                                ...
                                                if (count_lt(ob->query("balance"), money))
#else                                      
...
private string check_auction(object ob)
{
        ...
        if (ob->query("no_sell") || ob->query("no_drop") || ob->query("maze_item"))
        {
...
public void decide_auction(object me, string player)
{
        ...
        {
                map_delete(auction_info, id);
                message_auction(sprintf("由于%s(%s)的拍卖品突然失踪，此次拍卖被取消。", me->name(), id));           
                return;
        }
        ...
        case 2:
#ifdef LONELY_IMPROVED
                if (count_lt(who->query("balance"), money))
#else
