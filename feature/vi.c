...
protected void _append(object pl);        // 在游标後面加字
protected void _append_line(object pl);   // 在游标底下加一新行
protected void _backspace(object pl);     // 删除游标前的字
protected void _confirm_save(string str, object pl); // 离开编辑器前确认存档
protected void _del_char(object pl, int bs);      // 删除游标上的字
protected void _del_line(object pl);              // 删除整行
protected int  _dispatcher(object pl);            // 分派指令到不同函式
protected void _down(object pl);                  // 游标往下移一行
protected void _end(object pl);                   // 将游标移到行尾
protected void _esc(object pl);                   // 从输入模式回到命令模式
protected void _file_end(object pl);              // 将游标移到档尾
protected void _file_head(object pl);             // 将游标移到档头
protected void _goto_line(object pl);             // 跳到特定行去
protected void _goto_line_done(string str, object pl, int rein); // 取得行号
protected void _help(object pl);                  // 辅助画面
protected void _home(object pl);                  // 将游标移到行首
protected void _input(string str, object pl, int fresh); // 读得输入字串
protected void _insert(object pl);                // 在游标前面插字
protected void _insert_line(object pl);           // 在游标上方加一新行
protected void _insert_exclam_mark(object pl);    // 在游标前面插入一个「!」字元
protected void _join(object pl);                  // 将下行文字移到此行後面
protected void _keymap(object pl);                // 定义新功能键
protected void _keymap_done(string str, object pl); // 取得定义功能键
protected void _left(object pl);                  // 游标往左移一字
// 在状态列显示讯息
...
protected void _next_match(object pl);            // 搜寻/替换下个符合字串
protected void _page_down(object pl);             // 往下卷一页（22 行）
protected void _page_up(object pl);               // 往上卷一页（22 行）
protected void _process(object pl);               // 处理输入模式的字串输入
protected void _quit(mixed unused, mixed pl);   // 离开编辑器
protected void _refresh(object pl);               // 更新画面
protected void _refresh_cursor(object pl);        // 更新游标位置
// 重绘画面
protected void _refresh_screen(object pl, int s_row, int e_row, int s_col);
protected void _refresh_status(object pl);        // 更新状态列
protected void _replace(object pl);               // 字串替换
protected void _replace_done(string str, object pl); // 取得替换字串
protected void _right(object pl, int col, int extra);  // 游标往右移一字
protected void _search(object pl);                     // 字串搜寻
protected void _search_done(string str, object pl);  // 取得搜寻字串
protected void _tab(object pl);                        // 命令模式下 TAB 键的定位
protected void _undef_key(object pl);                  // 取消某功能键定义
protected void _undef_key_done(string str, object pl); // 取得指定功能键
protected void _up(object pl);                           // 游标往上移一行
protected void _write(object pl);                        // 取得储存档案名称
protected void _write_done(string str, object pl, int quit); // 储存档案

protected mapping EditedFiles=([ ]);              // 编辑中的档案
/*
