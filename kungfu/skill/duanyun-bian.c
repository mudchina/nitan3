...
int practice_skill(object me)
{
        ...
        if ((int)me->query("jing") < 50)
                return notify_fail("你的精气不够练断云鞭法。\n");
           if ((int)me->query("neili") < 50) 
                   return notify_fail("你的内力不够练断云鞭法。\n"); 
        me->receive_damage("jing", 10 );
        me->add("neili",-20);
        if( me->query_skill("duanyun-bian",1) > 150 ) me->receive_damage("jing", 5 );
        if( me->query_skill("duanyun-bian",1) > 180 ) me->receive_damage("jing", 10 );
        if( me->query_skill("duanyun-bian",1) > 200 ) me->receive_damage("jing", 15 );
