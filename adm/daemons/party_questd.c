// party_questd.c
// edit by smallfish@HUAXIA
// gongxin must be add in party
...
#define QUEST_NUM       10              // 每个任务列表的任务数量
...
void ready_to_start()
{
        ...
        all_user = filter_array(all_user, (: interactive($1) && 
                                !wizardp($1) && 
                                count_ge($1->query("combat_exp"), $2) &&
                                count_le($1->query("combat_exp"), $3) &&
                                environment($1) &&
                                !$1->is_ghost() &&
                                !$1->in_prison() &&
                                bunchp($1) :), 10000, 600000);
...
void start_quest()
{
        ...
        bunch_quest["type"] = "追杀";
        bunch_quest["ob"] = ob;
        bunch_quest["bonus"] = ob->query("bonus") * 2;
        bunch_quest["name"] = ob->name();
        bunch_quest["place"] = ob->query("place");
        bunch_quest["difficult"] = check_difficult(ob->query("lvl"));
        bunch_quest["reward"] = check_reward(ob->query("lvl"));
        ...
        if (random(2))	//黑道追杀
        {
                ...
                if (sizeof(all_user))
                        ...
                        {
                                tell_object(all_user[i], HIB"迎面急步过来一位〖" + all_user[i]->query("bunch/bunch_name")+
        ...
        else		//白道追杀
        {
                ...
                if (sizeof(all_user))
                        for (i = 0;i < sizeof(all_user);i++)
                        {
                                tell_object(all_user[i],HIW"迎面奔来一位〖"+all_user[i]->query("bunch/bunch_name")+
        ...
        if (ob)
        {
                ...
                ob->set("bunch_quest", bunch_quest);
        }
        return;
}
