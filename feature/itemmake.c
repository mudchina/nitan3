...
string weapon_long()
{
        ...
        if (attack_lvl == ULTRA_LEVEL)
        {
                ...
                if (query("ultimate/121"))
                        result += HIY + name() + HIY "的等级：太古神品  LV10\n" NOR;
                else
                if (query("ultimate/105"))
                        result += HIY + name() + HIY "的等级：远古神品  LV10\n" NOR;
                else
                if (query("ultimate/87"))
                        result += HIY + name() + HIY "的等级：上古神品  LV10\n" NOR;
                else        
...
int do_enchase(object me, object tessera)
{
        ...
        if (query("equipped"))
                return notify_fail("你先解除" + name() + "的装备再说！\n");
            ...
            (query("armor_type") && query("armor_type") == "hands")))
        {
                if (query("enchase/used") + 1 >= query("enchase/flute"))
                        return notify_fail(name() + "上的剩下的唯一的凹槽是用来镶嵌10lvl兵器的特殊物品。\n");
        }
        ...
        message_vision(HIM "听得“喀啦”一声，$N" HIM "将" + tessera->name() +
                       HIM "镶嵌到了$n" HIM "上面的凹槽里，\n只见" + tessera->name() + HIM
        ...
        if (ENCHASE_D->identify_ultimate_ob(this_object()))
                tell_object(me, BLINK HBMAG + tessera->name() + BLINK HBMAG "与" + name() + 
                                BLINK HBMAG "的魔力充分融合相生使" + name() + 
