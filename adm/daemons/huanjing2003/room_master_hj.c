...
void c_msg( string msg )
{
    ...
    else tell_object( naihe, HIR"〖幻境系统通知〗："+msg + "\n"NOR );
}
...
int dispersion( object me )
{
    ...
    CHANNEL_D->do_channel( me, "sys", sprintf("%s 手动执行幻境游戏主控中心派发随机物件函数(%d 个物件)。", me->query("id"), times));
...
int del_all( object me )
{
    ...
    tell_object( me, "Ok. 已执行函数，请留意 sys 信息。\n");
    CHANNEL_D->do_channel( me, "sys", sprintf("%s 手动执行幻境游戏主控中心的强制清除所有境内物件函数。", me->query("id") ));
    clean_hj();
    return 1;
}
