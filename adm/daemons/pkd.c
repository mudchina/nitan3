...
int join_competition(object ob)
{
        ...
        if (count_lt(ob->query("combat_exp"), 800000))
        ...
        for (int i = 0; i < sizeof(inv); i++)
        {
                ...
                return notify_fail("你背的是谁？还不快快放下！要不还是别进去了。\n");
        }
...
private void give_bouns(object me)
{
        ...
        // bouns
        pot = count_sub(me->query_potential_limit(), me->query("potential"));
        if (count_gt(pot, 50000)) pot = "50000";
        exp = count_add(count_div(me->query("combat_exp"), 500), 10000);
        if (count_gt(exp, 100000)) exp = "100000"; 
        sscanf(pot, "%d", pot);
...
private void send_invite_message()
{
        ...
        obs = filter_array(all_interactive(),
                           (: ! wizardp($1) &&
                              count_ge($1->query("combat_exp"), $(exp_dl)) &&
                              count_le($1->query("combat_exp"), $(exp_ul)) :));
        if (! sizeof(obs))
                return;
...
string reject_join(object me)
{
        ...
        if (count_lt(exp, tlist[selected]["exp"][0]))
                return "你还是算了吧，你这点经验就别进去现眼了。";

        if (count_gt(exp, tlist[selected]["exp"][1]))
                return "哦...您武功如此高强，这次还是别和小辈们争了。";

        if (me->query_condition("killer", 1))
                return "你正在被官府通缉，所以不能参加屠人大赛。";
