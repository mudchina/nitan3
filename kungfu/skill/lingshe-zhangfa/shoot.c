...
int perform(object me, object target)
{
	...
	if( me->query("neili") > random(target->query("neili")) && exp1 > random(exp2) ) {
                ...
                message_vision(HIR"那暗器去得好快，祗听$n一声惨嚎，尚未看清暗器来路，已被射中"+(arrayp(limbs)?limbs[random(sizeof(limbs))]:"身体")+"！\n"NOR, me, target);

		if( type == "怪蛇" || type == "灵蛇" )
                     target->affect_by("bt_poison",
                              ([ "level" : poi_amount/3 + random(poi_amount/3),
                                 "id"    : me->query("id"),
                                 "duration" : poi_amount/50 + random(poi_amount/50)]));
		else
                     target->affect_by("snake_poison",
                              ([ "level" : poi_amount/3 + random(poi_amount/3),
                                 "id"    : me->query("id"),
                                 "duration" : poi_amount/60 + random(poi_amount/60)]));
...
{
        if (!me || !me->query_temp("baituo/shoot")) return;
