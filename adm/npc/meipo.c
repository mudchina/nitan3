//NPC: /d/suzhou/npc/meipo.c 只处理结婚与拜堂
// smallfish 修改2001.06
...
int accept_object(object me, object ob)//交钱
{
    ...
    log_file( "static/MARRY", sprintf("%s(%s)结婚交钱 in %s。\n",me->query("name"),
...
int do_chase(object ob,object dest)//喜鹊送请柬
{
	...
	message_vision(HIY"\n天空中突然飞过来一只花喜鹊，落在$N肩头，将衔着的一张喜柬递给$N。\n" NOR, dest);
        ...
        obj->set_name(HIR+(string)ob->query("marry/me_name")+"的结婚请帖" NOR);
	obj->move(dest);
...
int do_marry(string arg) //判断结婚
{
    ...
    room = find_object("/d/suzhou/xifu-tang");
    if (! objectp(room))  room = load_object("/d/suzhou/xifu-tang");
    xlf = present("xi laifu", room);
    if (objectp(xlf))
         return notify_fail("媒婆笑道：“啊呀真不凑巧！喜老板正在主持婚宴，" +
                            "要不今天你们先去喝杯喜酒，明儿再给你们办？”\n");
...
*/
	if (((int)me->query("age") < 18) && (me->query("gender") == "男性")
		||((int)me->query("age") < 18) && (me->query("gender") == "无性")
	...
	if( mapp( me->query("couple")))
	{
		...
		if ( me->query("couple/have_couple") )
		{
			...
			if ((string)me->query("couple/couple_gender") == "丈夫")
	...
	if( mapp( obj->query("couple") ) )
	{
      if ((int)obj->query("couple/married")>2)
		...
		{
			if((string)obj->query("couple/couple_gender") == "丈夫")
	...
	{
		if ((string)obj->query("gender") == "女性" || ((string)obj->query("gender") == "无性" && (string)me->query("gender") == "男性"))
			message_vision(MAG "\n$N对著$n问道：“你愿意嫁给我吗？”\n\n"NOR, me,obj);
		else
			message_vision(MAG "\n$N对著$n问道：“你愿意娶我吗？”\n\n"NOR, me,obj);
	...
	obj->set_temp("pending/pro",1);//婚礼中
        ...
        me->set("couple/couple_name",(string)obj->name(1));
        me->set("couple/couple_id",obj->query("id"));
        obj->set("couple/couple_name",(string)me->name(1));
...
"			     ■■■  ■■■\n\n"NOR);
	...
	case 10:
  	  	...
  	  	if(me->query("couple/couple_gender")=="妻子" )
  	  	{
  	  		m_cloth->move(me);
  	  		w_cloth->move(obj);
  	  	}
...
int last_marry(object ob,object me,object obj)//拜堂结束
{
	...
	say("媒婆喊道：“轿夫！快将新人送到迎宾楼举行婚宴，我和月老就不去了，呵呵！”\n说完一扭一扭地向西边走去。\n");
     write_file( "/log/static/MARRY", sprintf("%s(%s) and %s(%s) married in %s。\n",me->query("name"),me->query("id"),
