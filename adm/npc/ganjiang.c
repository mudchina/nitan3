...
protected int   do_wash(string arg);
...
mapping   material = ([
        "玄铁石"   : "/d/item/obj/xuantie",
        "乌金丝"   : "/d/item/obj/wujins",
        "玄冰寒铁" : "/d/item/obj/hantie",
        "补天石"   : "/d/item/obj/butian",
        "冰蚕丝"   : "/d/item/obj/tiancs",
        "七色璎珞" : "/d/death/obj/qise",
        "寰宇天晶" : "/d/death/obj/tianjing",
]);
        ...
        if (info["type"] == "weapon")
        {
                ...
                if (info["stype"] == "throwing")
                {
                       buf += sprintf("\tset(\"base_weight\", %d);\n",
                                       item_temp->query_weight() * item_temp->query("item/wscale") / 10000);
...
int do_discard(string arg)
{
        ...
        if (ob->query("material_file"))
        {
                item = new(ob->query("material_file"));
                item->move(me, 1);
        } else
        {
                sscanf(filter_color(ob->query("long")), "这是由%s炼制而成%*s", desc);
                if (! undefinedp(material[desc]))
                        item = new(material[desc]);
                else
                        item = new("/d/item/obj/butian");  
                item->move(me, 1);
        }
        ...
        message_vision("$N拿出一" + item->query("unit") + item->name() + "交给"
                       "$n。\n", this_object(), me);
...
int do_notch(string arg)
{
        ...
        if (count_lt(me->query("balance"), 10000000))
                return notify_fail("你在钱庄里的存款不够支付道具开槽费用。\n");
                
        message_vision(CYN "$n拿出一" + ob->query("unit") + ob->name() +
        ...
        {
                command("heng");
                command("say 这不是我已经给开槽过了吗。");
                return 1;
        }       
        
        message_vision(HIM "$N接过$n" HIM "手中的" + ob->name() + HIM "，随手把它抛入炉"
                     "内，霎时间，那" + ob->name() + HIM "已被火炉中的熊" HIR "烈焰" HIM "吞没。\n"
                     "$N凝视着炉火，仰天叹道：天地之精华为其血脉，日月之光辉是其灵气......\n" NOR, 
                     this_object(), me);
        
        flag = 0;
        ob1 = present("dragon horn", me);
        if (objectp(ob1)) flag = 1;
        
        command("say 你要是有青龙之角这个开槽至宝就好了。");   
            ...
            ob->query("armor_type") == "hands"))
        {
                if (flag)
                {
                        ...
                        message_vision(HIC "$n赶紧把" + ob1->name() + HIC "取出必恭必敬交给$N。\n" NOR, this_object(), me);
                        destruct(ob1);
                }
                else              
                        ob->set("enchase/flute", 4 + random(4));
        }
        else
        {
                if (flag)
                {
                        ob->set("enchase/flute", 5);
                        message_vision(HIC "$n赶紧把" + ob1->name() + HIC "取出必恭必敬交给$N。\n" NOR, this_object(), me);
                        destruct(ob1);
                }
                else                                         
                        ob->set("enchase/flute", 3 + random(3));
        }
                
        ob->set("enchase/used", 0);
        ...
        message_vision(HIW "$N左手持蛟龙剪，一扬手已经取出了" + ob->name() +
                       HIW "，右手持" + (flag ? "青龙之角" : "金刚钻") + "聚精会神地用力敲打着，\n片刻间，" + ob->name() +
                       HIW "上面豁然多了" HIY + chinese_number(ob->query("enchase/flute")) + 
                       HIW "个圆形的小孔。\n\n" NOR, this_object(), me);
...
int do_wash(string arg)
{
       ...
       object me, ob;
        ...
        if (! objectp(ob = present(arg, me)))
        {
                command("say 你身上没有这种东西。");
                return 1;
        }

        if (ob->query("equipped"))
        {
                command("say 你先解除" + ob->name() + CYN "的装备再说！");
                return 1;
        }
                
        message_vision(CYN "$n拿出一" + ob->query("unit") + ob->name() +
                       CYN "对$N说：我想请您帮把" + ob->name() + CYN "凹槽里的宝石清理掉。\n" NOR,
                       this_object(), me);

        if (! ob->is_item_make() && ! ob->query("unique"))
        {
                command("heng");
                command("say 我且能给这如此平凡之物清理凹槽里物品？！");
                return 1;
        }
        ...
        if (! ob->query("armor_type") && 
            ! ob->query("skill_type"))
        {
                command("say 我只给兵器或防具类型的道具处理凹槽的宝石。");
                return 1;
        }
        ...
        {
                command("heng");
                command("say 这还没有开槽过了呢。");
                return 1;
        }       
        
        if (! ob->query("enchase/used"))
        {
                command("heng");
                command("say 这上面没有镶嵌过东西。");
                return 1;
        }       
        ...
        message_vision(RED "炉火淡淡的暗了下来，呈现出一片红色......\n" NOR, this_object(), me);
        message_vision(HIW "$N左手持蛟龙剪，一扬手已经取出了" + ob->name() +
                       HIW "，右手持金刚钻聚精会神地用力敲打着，\n片刻间，" + ob->name() +
                       HIW "上面" HIY + chinese_number(ob->query("enchase/flute")) + 
                       HIW "个凹槽内已空无一物，犹如新出。\n" NOR, this_object(), me);
                
        f = ob->query("enchase/flute");
        ob->delete("enchase");
        ob->delete("magic/power");
        ob->delete("magic/type");
        ob->delete("magic/tessera");
        ob->delete("ultimate/69");
        ob->delete("ultimate/87");
        ob->delete("ultimate/105");
        ob->set("enchase/flute", f); 
        if (ob->query("material") == "tian jing")
                ob->set("enchase/SN", 8);
        if (ob->query("material") == "noname object")
                ob->set("enchase/SN", 16);
        ob->save();
