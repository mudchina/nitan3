...
void create()
{
    set("short", "龙门渡口");
        set("long", @LONG
渡口前就是汹涌澎湃的黄河(he)了。浊流滚滚，泥沙俱下，声震数里。两岸
渡船来来往往，在波谷中穿行，甚为凶险。过了黄河，就是陕西了。
LONG );
    set("exits", ([
        "northeast" : "/d/heimuya/road4",
    ]));
    set("item_desc", ([
...
void check_trigger()
{
    ...
    if(!query("exits/enter") ) {
        if( !(room = find_object("/d/heimuya/duchuan")) )
            room = load_object("/d/heimuya/duchuan");
        if( room = find_object("/d/heimuya/duchuan") ) {
            if((int)room->query("yell_trigger")==0 ) {
                room->set("yell_trigger", 1);
                set("exits/enter", "/d/heimuya/duchuan");
...
void arrive()
{
    ...
    if( room = find_object("/d/heimuya/duchuan") )
    {
        room->set("exits/out", "/d/heimuya/dukou2");
