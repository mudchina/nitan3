...
int form_array(object me,string arg)
{
	...
	if (enemy->query("race") != "人类")
        ...
        { 
           ...
           if (member[i]->query("family/family_name") != "全真教") 
...
void start_beidouzhen(object ob, int mypos,int beidou_time,int bd_power )
{
	...
	ob->set_temp("beidou_time", beidou_time);
	ob->set_temp("bd_pos", mypos);
	ob->set_temp("bd_power",bd_power);
	ob->add_temp("apply/defense",bd_power);
	ob->add_temp("apply/parry",bd_power/3);
	ob->add_temp("apply/dodge",bd_power/3);
...
int beidouzhen(object me)
{
	...
	beidou_time = (int)me->query_temp("beidou_time");
	where = environment(me);
	mypos = me->query_temp("bd_pos");
	...
	if ( me->query("neili") < 300 || me->query("jingli") < 100 )
	{
		out_beidou(me);
		return 0;
	}
	...
	for ( i = 0;i < sizeof(member);i++)
	{
		...
		if (!member[i]->query_temp("pending/beidou"))
		{
			ok = 0;
			break;
		}
	...
	power = me->query_temp("bd_power",1);

	if( random(6)==1 )
	{
		tell_room(environment(me), HIR"\n" + zhen[(int)random(4)] +"\n"NOR, this_object());
		if( objectp(enemy) && 
		    objectp( present( enemy->query("id"),environment(me) )) )
	...
	if( random(2)==1 )
	{
		...
		me->add_temp("apply/sword",power/2);
...
{
	start_beidouzhen(me, me->query_temp("bd_pos"), me->query_temp("beidou_time"), me->query_temp("bd_power") );
}
...
void out_beidou(object me)
{
	...
	bd_power = me->query_temp("bd_power");
	...
	me->add_temp("apply/defense",-bd_power);
	me->add_temp("apply/parry",-bd_power/3);
	me->add_temp("apply/dodge",-bd_power/3);
