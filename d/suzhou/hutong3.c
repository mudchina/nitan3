...
void create() {
        set("short", "胡同");
        set("long", @LONG
这是一条黑暗、肮脏的小巷，空气中仿佛飘扬着一股罪恶的气息，几个流氓
在游荡，向东南就能回到大街，你还是赶快走吧。旁边有个木门（door）。 
LONG );
        set("outdoors", "suzhou");
        set("no_clean_up", 0);
        set("exits", ([
...
int do_knock(string arg)
{
        ...
        if (!arg||arg=="") 
          ...
          if( arg=="door" ) {
                ...
                if (me->query("bunch/bunch_name")=="修罗门")
                {
...
说完一把把$N拉了进去。\n"NOR, me);
                ...
                if (me->query("bunch/bunch_name")!="修罗门" && (int)me->query("shen") <= 0)
                {
