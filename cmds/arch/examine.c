...
protected string examine_player(string name, int copy_user, int raw, int last_touched)
{
       ...
       // int age;
        ...
        if (raw)
        {
                ...
                if (day >= 180 && ! objectp(find_player(name)))
                {
        ...
        if (copy_user)
              login_ob->set_temp("restore_mysql", 1);
        ...
        if (! objectp(user_ob = find_player(name)))
        {
                ...
                if (copy_user)
                        user_ob->set_temp("restore_mysql", 1);
...
protected string is_illegal(object ob)
{
        ...
        else
        {
                ...
                gold = atoi(count_div(ob->query("balance"), 10000));
        ...
        dexp = count_sub(exp, ob->query("last_examine/combat_exp"));
        dpot = count_sub(ob->query("potential"), ob->query("last_examine/potential"));
        ...
        if (count_gt(gold, 2000))
                return HIY "拥有 " + gold + " 两黄金" NOR;

        if (count_lt(exp, 1000000) && count_ge(gold, 1000))
                return HIC "经验 1M 以内拥有 " + gold + " 两黄金" NOR;

        if (count_gt(exp, 10000) && dt > 60)
        {
                if ((sp = atoi(count_div(count_mul(dexp, 60), dt))) > 90)
                        return HIR "平均经验速度太快(" + sp + "/分钟:" + dt / 60 + "分钟)" NOR;

                if ((sp = atoi(count_div(count_mul(dpot, 60), dt))) > 60)
