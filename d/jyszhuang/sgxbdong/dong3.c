...
void create()
{
        set("short", HIB"寒魄"NOR);
        set("long", @LONG
这里是上古时代保存一个神秘洞穴，据说里面陈列着上古时代遗留下来的
一把名为寒魄的宝刀，洞中有些黑呀呀的，不知暗伏着什么机关。    
LONG );
        set("indoors", "city");
        set("exits", ([
...
int do_break(string arg)
{
       ...
       while (i--)
       {
        ...
        if(ilist[j]->query("biwu") && ilist[j]->query("id_id")=="han-po dao")
        {
         ob=ilist[j];
       ...
       ob=new(__DIR__"obj/han-po");
       ob->move(environment(me));
       message_vision(
       HIB"$N运足掌力，猛地朝洞壁击去，只听轰的一声巨响，一道蓝光\n"
       +"夺壁而出，嚓的一声落到了你的眼前！\n"NOR,me);    
