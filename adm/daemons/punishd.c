...
void monitor()
{
	...
	if (sizeof(obs))
        {
                obs = sort_array(obs, (: atoi(count_div($2->query("combat_exp"), 100)) -
                                         atoi(count_div($1->query("combat_exp"), 100)) :));
        ...
        obs = filter_array(users(),
                           (: count_lt($1->query("combat_exp"), 20000000) &&
...
void family_punish()
{
        ...
        obs = filter_array(users(), (: count_ge($1->query("combat_exp"), 100000) &&
                                       environment($1) &&
