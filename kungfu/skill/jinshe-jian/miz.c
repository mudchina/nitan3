...
// Create by 心荻(Xiaot) at Tue Mar 14 15:49:54 2006
// 蛇影迷踪(miz)

#include <ansi.h>
#include <combat.h>

#define PFM "「" HIW "蛇影迷踪" NOR "」"
...
int perform(object me, object target)
{
        ...
        if ((int)me->query_skill("jinshe-jian", 1) < 400)
                return notify_fail("你" + to_chinese("jinshe-jian") + "不够娴熟，难以施展" PFM "。\n");

        if (member_array("sword", weapon_sk) == -1)
        {
                if (me->query_skill_prepared("sword") != "jinshe-jian")
                        return notify_fail("你没有准备" + to_chinese("jinshe-jian") + "，难以施展" PFM "。\n");
        }
        ...
        msg = HIW "$N一声轻嗤，手中武器化为一条灵蛇，只见一道银线，忽啸九天，来无影去无踪，$n心头一震，脸色都黯淡下来！" + "\n" + NOR;
        ...
        {
                msg += HIM "只见一道银线忽闪，$n已然惨叫，全身上下伤痕累累，鲜血汹涌喷出！" + "\n" NOR;
                count = ap / 10;
                me->add_temp("apply/attack", count);
                me->add_temp("apply/damage", count);
        } else
        {
                msg += NOR + CYN "$p见势不妙，急忙一掌击向地面，挖出一个狗洞，爬进去躲了起来！" + "\n" NOR;
                count = 0;
        }
