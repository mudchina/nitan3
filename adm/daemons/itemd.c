...
void killer_reward(object me, object victim, object item)
{
        ...
        if (count_lt(exp, 10000) || count_lt(me->query("combat_exp"), count_div(count_mul(exp, 4), 5)))
                return;

        exp = count_div(exp, 10000);
        if (count_gt(exp, 250)) exp = count_add(100, count_div(count_sub(exp, 250), 16));
        else
        if (count_gt(exp, 50)) exp = count_add(50, count_div(count_sub(exp, 50), 4));
        my_id = me->query("id");
        o = item->query("owner");
        if (! o) o = ([ ]);
        if (! undefinedp(o[my_id]) || sizeof(o) < 12)
                o[my_id] = count_add(o[my_id], exp);
        else
        {
                ...
                for (i = 1; i < sizeof(ks); i++)
                        if (count_gt(o[ks[lowest]], o[ks[i]]))
                                lowest = i;
...
int receive_summon(object me, object item)
{
        ...
        if (item->query("skill_type"))
        {
                ...
                {
                        HAND_CMD->main(me, item->query("id"));
                        return 1;
                }
...
int do_enchase(object me, object item, object tessera)
{
        ...
        if (! item->query("enchase/flute"))
                return notify_fail(item->name() + "上没有凹槽用来镶嵌宝石。\n");

        if (item->query("enchase/used") >= item->query("enchase/flute"))
                return notify_fail(item->name() + "上的凹槽已经被镶满了。\n");
        ...
        message_vision(HIM "“喀啦”一声，$N" HIM "将" + tessera->name() + "镶嵌到了$n" HIM "上面，\n只间$n"
                       HIM "上隐隐的显过了一道奇异的光芒，随即变得平静，说不出的平凡。\n\n" NOR, me, item);
        ...
        CHANNEL_D->do_channel(this_object(), "rumor",
                              "听说神品" + item->name() + HIM +
                              "来到了人间。");
        item->add("enchase/SN", tessera->query("enchase/SN")); 
...
mixed weapon10lv_hit_ob(object me, object victim, object weapon, int damage_bonus)
{
        ...
        case "lighting":
                ...
                add = me->query_temp("apply/add_lighting");
                reduce = victim->query_temp("apply/reduce_lighting");
                avoid = victim->query_temp("apply/avoid_lighting");
                damage += damage * add / 100;
                damage -= damage * reduce / 100;
                
                if (random(100) < avoid || damage < 0) return;
        ...
        case "cold":
                ...
                damage = (power + jingjia) * 200 / (100 + resistance);
                add = me->query_temp("apply/add_cold");
                reduce = victim->query_temp("apply/reduce_cold");
                avoid = victim->query_temp("apply/avoid_cold");
                damage += damage * add / 100;
                damage -= damage * reduce / 100;
        ...
        case "fire":
                ...
                add = me->query_temp("apply/add_fire");
                reduce = victim->query_temp("apply/reduce_fire");
                avoid = victim->query_temp("apply/avoid_fire");
                damage += damage * add / 100;
                damage -= damage * reduce / 100;
        ...
        case "magic":
                ...
                damage = (power + jingjia) * 200 / (100 + resistance);
                add = me->query_temp("apply/add_magic");
                reduce = victim->query_temp("apply/reduce_magic");
                avoid = victim->query_temp("apply/avoid_magic");
                damage += damage * add / 100;
                damage -= damage * reduce / 100;
...
void reduce_consistence(object item)
{
        ...
        if (item->query("material") == "noname object")
                return;
