...
int do_take(string arg)
{
        ...
        if (ob->query_amount()) 
        {
                ...
                ob->move(me, 1);
                save();                                
                message_vision("$N从箱子里取出一" + ob->query("unit") + ob->query("name") + "。\n", me);
                return 1;
        }
...
int store_item(object me, object ob, int amount)
{
        ...
        {
                tell_object(me, "箱子不保存" + ob->query("name") + "，请你自己妥善处理。\n");
                return 0;
        }
        ...
        {
                tell_object(me, "箱子不保存" + ob->query("name") + "，要存钱的话请去钱庄吧。\n");
                return 0;
        }
        ...
        {
                tell_object(me, "请你先把" + ob->query("name") + "里面的东西先拿出来再存放。\n");
                return 0;
        }
