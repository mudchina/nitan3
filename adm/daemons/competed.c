...
protected void auto_check()
{
        ...
        for (i = 0; i < sizeof(total); i++)
        {
                ...
                if (! sscanf(room, "/d/city/lunjia%*s"))
                {
                        // not in pk room
...
int join_competition(object ob)
{
        ...
        if (time() - (int)ob->query_temp("competed") < 500)
                return notify_fail("你刚打过擂台不久，你还是等会再来吧。\n");
                
        exp = ob->query("combat_exp");
        if (count_lt(exp, 100000))
...
protected int finish_competition()
{
	...
	challenger->set_temp("competed", time());
