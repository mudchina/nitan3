...
void create()
{
        set("short", "观云台");
        set("long", @LONG
这里是武当山天柱峰的观云台。放眼望去，只见云海涌动，气势磅礴。你心
中微微一动，凝神静思，感觉在这里进行参悟(canwu)，静观云的变化，于太极拳
理似乎极有帮助。
LONG );
        set("no_fight", "1"); 
        set("no_steal", "1"); 
...
int canwu(object me)
{
        ...
        if (!me->query("taiji_quanli",1) || me->query("taiji_quanli",1) < 2 )
               hint = 10000;
        else  if (me->query("taiji_quanli",1) < 3 )   hint = 20000;
              else  hint = 30000;
