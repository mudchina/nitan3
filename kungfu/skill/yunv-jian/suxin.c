...
int perform(object me, object target)
{
	...
	message_vision(HIW "$N突然面色庄重，双目清澄，手中" + 
                       weapon->query("name") + HIW + "蒙上了一层白霜，寒气逼人！\n\n" NOR,me);
	message_vision(HIW "$N腾身而起，姿态无比婀娜飘逸，挥剑向$n直刺过去，快愈流星！\n\n"NOR,me,target);
        ...
        if (ap/2 + random(ap) > dp || !living(target))
	{
		...
		msg = HIR "只听$n一声惨呼，已然给$N的" + NOR + weapon->query("name") + HIR +
