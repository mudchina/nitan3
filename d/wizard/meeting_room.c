...
心(sinb)；泥潭㈡巫师画像：一人两角(yezh)、大脸猫(dump)；泥潭
㈢ 巫师画像，分别有：风飞(wind)、柒叶(qiy) 、南海钓客(jak)、
随意(dream)、天下无雪(hhm)等等。屋子的正中央放着一张楠木桌椅，
是泥潭巫师组用来开会用的。
LONG );
...
int do_start(string arg)
{
        ...
        foreach (object player in users())
        {
                ...
                player->set_temp("disable_type", HIW "<会议中>" NOR);                                 
        }
...
int do_over()
{
        ...
        foreach (object player in users())
        {
                ...
                if (player->query_temp("disable_type") ==  HIW "<会议中>" NOR)       
