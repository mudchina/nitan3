...
int do_show(string arg)
{
       ...
       // string fn, filename;
        ...
        if (! wizardp(me) && ! me->query_temp("zhangpaid/张天师", 1))
        {
                command("heihei");
        ...
        if (ob->is_enchase_ob() && ob->query("no_identify"))
        {
                if (! (filename = ENCHASE_D->identify_ob(me, ob)))
                {
                        command("say 你的物品乃我所从未见过，叫巫师来处理吧。");
                        return 1;
                }
                // ob1 = get_object(filename);
                ob1 = new(filename);
                ...
                /*
                fn = base_name(ob) + ".c";
                if (sscanf(fn, "/data/enchase/%*sjewel%d-%d.c", n, d) == 2)
                        rm(fn);
                */
                destruct(ob);
                ob1->move(me, 1);
                command("say 行了，你的宝物我给鉴定完了，拿去吧。");
                me->delete_temp("zhangpaid/张天师");
                return 1;
        }
