...
void lost()
{
        ...
        scorei = random(atoi(count_div(query("combat_exp"), 500))) + 10;
        weiwangi = random(atoi(count_div(query("combat_exp"), 200))) + 10;
        ...
        ob->add("family/gongji", poti);
        tell_object(ob, sprintf(HIG "你获得了%s点经验、%s点潜能，并增"
                                "加了%s点威望、%s点江湖阅历和%s点门派功绩。\n" NOR,
                        chinese_number(expi),
                        chinese_number(poti),
