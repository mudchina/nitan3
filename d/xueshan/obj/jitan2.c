...
void create()
{
        set_name(HIY"般阳法坛"NOR, ({ "fa tan", "altar", "tan" }) );
        set_weight(300000);
...
int do_light(string arg)
{
        ...
        if (!id(arg))
                return notify_fail("你要点燃什么？\n");
        if(this_object()->query("pending") != me->query("name") )
...
void rise(object me)
{
        ...
        log_file("job/xueshan", me->query("name") + "(" + me->query("id") + ")得到" + me->query_skill("lamaism", 1)/100 + "点最大内力值" + ctime(time()) + "\n");
        }
...
//************************************
       ...
       //expG = gaining + random(gaining/2);
        expG = gaining*2 + random(gaining);
         // if (me->query_temp("award_pending"))
            ...
            me->add("potential", expG / 5 );
        log_file("job/xueshan",me->query("name") +" (" + me->query("id") + ") 烧人获得 " + 
            expG + " 点经验和 "+ expG / 5 + " 点浅能 "+ctime(time())+"\n");
       ...
       // if (me->query_temp("award_doubling"))
        ...
        if (random(me->query("potential") - me->query("learned_points")) > me->query_con()*10)
        me->unconcious();
