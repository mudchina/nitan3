...
int accept_object(object who,object ob)
{
       ...
       if (!who->query("bunch_quest") ||
           who->query("bunch_quest/type") != "调解纠纷" ||
           who->query("bunch_quest/ob") != this_object())
       ...
       exp = who->query("bunch_quest/bonus");
       exp += random(exp);
       pot = exp/(2+random(1));
       weiwang = 5 + random(10);
       score = 5 + random(10);

       who->delete("bunch_quest");
       who->add_temp("finish_bunch_times",1);
