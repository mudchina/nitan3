...
message_vision("$NµÄÍ·×²ÔÚÌ¶±ÚÉÏ£¬Ò»ÏÂ×Ó»èÁË¹ıÈ¥£¡\n", me); 
                call_out("be_saved",8);
                me->unconcious();
                room = find_object("/d/dragon/spirit10");
                 if(!objectp(room)) room=load_object("/d/dragon/spirit10");
                me->move(room);
                break;
                case 2:
message_vision("$N×ß½üÉîÌ¶Ì½Í·ÏòÀïÍûÈ¥¡£°¥Ñ½£¡Ò»²»Ğ¡ĞÄµôÁË½øÈ¥£¡\n", me);
              room = find_object("/d/dragon/underwater");
                if(!objectp(room)) room=load_object("/d/dragon/underwater");
                me->move(room);
                break;
        }
