...
void set_from_me(object me,int arg)
{
	...
	npc->set("family",me->query("family"));
        ...
        my["dex"] = hp_status["dex"];
                ...
                myexp = atoi(count_div(myexp, 1000));
	...
	{
		switch (me->query_temp("weapon")->query("skill_type"))
		{
			case "sword" :
...
void check_quest(object ob)
{
	...
	if (ob->query("quest_dg"))
		quest = ob->query("quest_dg");
	else if (ob->query("quest_kh"))
		quest = ob->query("quest_kh");
	else if (ob->query("quest_hs"))
		quest = ob->query("quest_hs");
	else if (ob->query("quest_sn"))
		quest = ob->query("quest_sn");
