...
void create()
{
        ...
        set("str", 20);
        set("dex", 20);
   set("long", "他就是冯正东，苏州现任知府，因红娘庄在其辖境，亲手负责玩家离婚之事。\n");
...
int accept_object(object me, object ob)
{
   if (!ob->query("money_id"))
             return notify_fail("冯正东冷冷地看了你一眼道：“是不是活得不耐烦了，敢拿本知府开玩笑？！”\n");
          if(ob->value() < 1000000)
              return notify_fail("冯正东皱了皱眉头说道：“你记性不好还是怎么着？100 gold，少一文也别想办！”\n");
        message_vision("冯正东接过钱点点说：“好吧！我们来办理吧！”\n",me);
...
int do_chat()
{
   ...
   message_vision(HIW "冯知府脸色一沉，惊堂木一拍喝道：“大堂之上，竟敢喧哗！来人呀！”\n"
   "“在！”两边衙役齐声应道。冯正东一指$N喝道：“把他给我重打二十大板！”\n"NOR,me);
   call_out("zou",1,me);
   return 0;
}
...
int do_unmarry(string arg)
{
        ...
        if (me->is_busy())
                return notify_fail("冯知府眉头一皱说道：“你看看你自己那个忙样！”\n");

        if( me->is_fighting() )
        {
                if( !this_object()->is_killing(me) )
                        this_object()->kill_ob(me);
                return notify_fail("冯正东怒火中烧：“大胆逆贼，来来来，本官送你去阴间去离婚！”\n");
        }

        if( this_object()->is_fighting() )
                return notify_fail("冯知府说道：“你先退下，等本官处理了这里捣乱的人再说！”\n");
   ...
   if(me->query_temp("unmarry"))
           return notify_fail("冯正东黑着脸看看你：“本官不是刚刚帮你们办过离婚手续？还来烦我！？”\n");

        if( !mapp( me->query("couple") ) || !me->query("couple/have_couple") )
        {
                if ((string)me->query("gender") == "女性")
                        say("冯正东脸一沉喝道：“大胆刁妇，你分明没有丈夫，还敢戏弄本官，来人呀！打她二十大板！”\n");
                else
                        say("冯正东脸一沉喝道：“大胆刁民，你分明没有妻室，还敢戏弄本官，来人呀！打他二十大板！”\n");
      call_out("zou",1,me);
      return 1;
        }
   if(!arg||arg!=(string)me->query("couple/couple_id"))
           return notify_fail("冯知府皱了皱眉头：你要和谁离婚？\n");
//   ob = new(LOGIN_OB);
...
//        if( !ob->restore() )
   ...
   {
       message_vision("冯正东听了$N的陈述，仔细查看了案卷，点点头道：“既然你的爱人已经不在人世，我就给你\n"
   ...
   obj->set_temp("unmarry",1);
        message_vision(HIY "冯正东看了看点点头说道：“$N和$n，既然你们都同意离婚，那本官也就给你们办了！”\n"
           "说完，在姻缘簿上重重地划去了两个名字。\n" NOR, obj, me);
        CHANNEL_D->do_channel(this_object(), "chat",
