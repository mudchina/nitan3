...
/*
		  if( interactive(ob = this_player()) &&
				...
				random(ob->query_kar() + ob->query_per()) < 30 &&
                                count_gt(ob->query("combat_exp"), count_div(count_mul(me->query("combat_exp"), 9), 10)) )
		  {
...
//        max = pow( query("combat_exp")*10.,0.33333);
        max = to_int(pow( to_float(atoi(count_div(query("combat_exp"), 100))), 1 / 3)) * 10;
	max1 = (int)max;
	...
	map_skill("force", "hamagong");
        ...
        if( count_lt(owner->query("combat_exp"), 4000))
	{
		  set_temp("apply/attack", 35);
		  set_temp("apply/damage", 15);
		  set_temp("apply/armor", 100);
	}
        else if( count_lt(owner->query("combat_exp"), 30000))
	{
		  set_temp("apply/attack", 45);
		  set_temp("apply/damage", 26);
		  set_temp("apply/armor", 100);
	}
        else if( count_lt(owner->query("combat_exp"), 60000))
	{
		  set_temp("apply/attack", 45);
		  set_temp("apply/damage", 36);
		  set_temp("apply/armor", 120);
	}
        else if( count_lt(owner->query("combat_exp"), 300000))
	{
...
private int is_suitable(object victim, object killer)
{
		  ...
		  if(  !clonep(victim) || userp(victim)
			...
			|| strsrch(base_name(victim), "/d/bwdh/") != -1
                        || count_lt(victim->query("combat_exp"), count_div(killer->query("combat_exp"), 15))
                        || count_gt(victim->query("combat_exp"), count_div(count_mul(killer->query("combat_exp"), 7), 3))
...
void setup_skill(object me,object obj)
{
	...
	if(!objectp(me) || !objectp(obj) )return;
        ...
        if( count_lt(obj->query("combat_exp"), me->query("combat_exp")) )
	{
		obj->set("combat_exp", me->query("combat_exp") /100  * ( 100+random(60) )  );
	}

        level = to_int(pow( to_float(atoi(count_div(obj->query("combat_exp"), 100))), 1 / 3)) * 10;
	if(level < 10) level = 10;

        if( count_lt(obj->query("combat_exp"), 4000) )
	{
		level = level/2 + random(level/2);
	}
        else if( count_lt(obj->query("combat_exp"), 30000))
	{
		level = level/2 + random(level/3);
	}
        else if( count_lt(obj->query("combat_exp"), 60000))
	{
		level = level/2 + random(level/3);
	}
        else if( count_lt(obj->query("combat_exp"), 300000))
	{
...
//	if (!living(ob)) return notify_fail("这不是活物！\n");
	...
	if ( query("food") >= 200  )
	{
        ...
        if( count_lt(ob->query("combat_exp"), count_div(count_mul(me->query("combat_exp"), 4), 5)) )
...
int convert(string arg)
{
	...
	if(me->query("jingli")<50)return notify_fail("你的精力不足。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if( (owner=present(me->query("owner"),here)) )
	{
		...
		exp = 4 + random(6 );
               if ( count_gt(me->query( "combat_exp" ), 6400000) ) exp = exp/2; 
		pot = 2 + random(4);
               if ( count_gt(me->query( "combat_exp" ), 6400000) ) pot = pot/2; 
		lvl = 3 + random(4);
		...
		if( owner->query("potential") > owner->query_potential_limit() )
			owner->set("potential",owner->query_potential_limit() );
...
int auto_perform()
{
	...
	int damage,bonus;
	int exp, pot, lvl;
    int zero_exp = atoi(count_div(target->query("combat_exp"), 100)) * 100;
	...
	message_vision("\n$N"+HIW"突然闪电般地跃起，象一条白箭似的射向$n"+HIW"的咽喉。\n"NOR,me,target);
        ...
        if( random( atoi(count_div(me->query("combat_exp"), 100)) ) > random( atoi(count_div(target->query("combat_exp"), 100)) ) || !living(target) )
	{
