...
void create()
{
	set("short", "蛇场");
        set("long", @LONG
这是欧阳世家所建的蛇场，从蛇谷捕捉来的毒蛇都在此饲养，四周围以丈许
来高的灰色圆墙，墙脚砌成圆弧形，光滑无缝。场里建了许多蛇窝，由欧阳世家
中的蛇奴负责管理，许多剧毒的怪蛇就是在此杂交而得。
LONG );
...
string snake_list(object obj, int flag)
{
	string str;
        ...
        sp = count_mul(obj->query("sk_poison"), 2);
//        sp = (int)obj->query("sk_poison")*2;

	if ( obj->query("race") == "蛇类" 
	&& obj->query("caught") == this_player()->query("id") ) {
                str = sprintf("%-24s： 经验：%-10s  蛇毒：%3d  \n" NOR,
