...
void set_from_me(object me, int scale)
{
        ...
        */
        ...
        if (count_gt(my["combat_exp"], 200000) && random(100) < 5)
        {
                // 经验 > 200K，5%几率出现多个敌人
...
int do_nod(string arg)
{
        ...
        if (! arg || arg != ob->query("id") || 
            ! me->query_temp("family_quest/waiting"))
                return 0;

        me->command("nod");

        if (me->query_temp("family_quest/ob"))
        {
                ...
                me->delete_temp("family_quest");
                destruct(ob);
                return 1;
        }       
        amount = me->query_temp("family_quest/waiting");
...
int filter_to_bonus(object ob, object aob)
{
        ...
        // 协助的对象一致
        if (aob == ob->query_temp("quest/assist"))
                return 1;
...
void random_place(string msg)
{
        ...
        if (count_lt(query("combat_exp"), 500000))
                not_place += ({ "大理一带" });

        if (count_lt(query("combat_exp"), 800000))
                not_place += ({ "西域" });
        ...
        if (! query_temp("is_helper") && ! query_temp("help_ob"))
        {
                    ...
                    ! me->query_temp("family_quest/waiting"))
                {
                        ...
                        case 1: 
                                ...
                                me->set_temp("family_quest/waiting", amount);
                                return;
                        case 2:
...
void die(object killer)
{
        ...
        if ((n = sizeof(obs)) > 0)
        {
                ...
                foreach (hob in obs)
                {
                        ...
                        // 看到奖励。
                        QUEST_D->delay_bonus(hob, ([ "exp"     : exp + random(4) +
                                                    ((hob == dob) ? extra_exp : 0),
                                                    "pot"     : pot + random(3) +
                                                    ((hob == dob) ? extra_pot : 0),
        ...
        if ((query_temp("is_helper") || query_temp("help_ob") ||
            query_temp("escape_times")) && random(2))
        {
