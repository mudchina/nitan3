...
int give_quest()
{
        ...
        */
        ...
        if (questdg["type"] == "杀" && (random(4) == 2 || wizardp(ob)))
        {
...
int accept_object(object who, object ob)
{
      ...
      int exp, pot,level;
       ...
       string msg;
        ...
        exp = level*315 + questdg["bonus"]*9;
        if (random(2))
        exp += random(who->query("questdg_times"));
        exp /= i;
        ...
        quest_count = who->query("questdg_times")%500;
        if (quest_count == 50)
        {
                msg = "$N呵呵笑了两声道：“你还可以，连着"+chinese_number(who->query("questdg_times"))+
                      "次任务都完成的漂漂亮亮，继续努力！”\n";
	        special = 1;
        } else
        if (quest_count == 100)
        { 
                msg = "$N笑的合不拢嘴，看了你一眼道：“有志气！"+chinese_number(who->query("questdg_times"))+
                      "次任务干得干净利索！”\n";
                special = "/d/shaolin/obj/puti-zi";
        } else
        if (quest_count == 150)
        {
                msg = "$N叹道：“真是长江后浪推前浪，想不到你接连"+chinese_number(who->query("questdg_times"))+
                      "次任务都无一疏漏，不易，不易！”\n";
                money = 1;
        } else
        if (quest_count == 200)
        ...
        if (quest_count == 350)
        {
                msg = "$N吓了一跳：“看来你真的很适合学武，竟接连"+chinese_number(who->query("questdg_times"))+
                      "次任务都无一疏漏，不易，不易！”\n";
	        special = "/clone/gift/tianxiang";
        } else
        if (quest_count == 400)
        {
                msg = "$N叹道：“真是一个武学天才啊，想不到你接连"+chinese_number(who->query("questdg_times"))+
                      "次任务都无一疏漏，不易，不易！”\n";
                special = "/clone/gift/jiuzhuan";
        } else
        if (quest_count == 450)
        {
                msg = "$N大声叫起来：“天才！真是天才，真有人连续"+chinese_number(who->query("questdg_times"))+
                      "次任务都无一疏漏，不易，不易！”\n";
                special = "/clone/gift/jiuzhuan";
        } else 
        if (quest_count == 0)
        {
                msg = "$N深深叹了口气，道：“没想到你连着"+chinese_number(who->query("questdg_times"))+
        ...
        if ((quest_count % 10) == 0)
        {
                msg = "$N呵呵笑着：“你表现很不错啊，居然连着" +
                      chinese_number(who->query("questdg_times")) +
                      "次都没有失手，别忙着练，慢慢来。”\n";
        }
        if (special) 
        {
        ...
        {
                obj = new("/clone/enchase/MAGjewel.c");
