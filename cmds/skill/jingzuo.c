...
int main(object me, string arg)
{
        ...
        if (time() - (int)me->query("jingzuo_time") < 120)
...
void wakeup(object me, object where)
{
        ...
        exppot = atoi(count_div(me->query("combat_exp"), 100000));
        ...
        if (count_ge(me->query("combat_exp"), 1500000))
                addc /= 2;

        if (count_ge(me->query("combat_exp"), 3000000))
                addc /= 2;

        if (count_ge(me->query("combat_exp"), 5000000))
                addc /= 2;
