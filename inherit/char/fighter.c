...
int accept_fight(object ob)
{
        ...
        if (count_lt(ob->query("combat_exp"), 12000))
...
/* delete and copy skill prepares */
        ...
        if (! undefinedp(my["min_exp"]) &&
            count_lt(my["combat_exp"], my["min_exp"]))
                my["combat_exp"] = my["min_exp"];

        if (count_gt(my["combat_exp"], my["max_exp"]))
...
int accept_kill(object ob)
{
        ...
        if (! me->query("damaged") && count_lt(ob->query("combat_exp"), me->query("combat_exp")))
        {
