...
void create()
{
	set_name("掌锤童子", ({ "zhangjian tongzi", "tongzi", "zhangjian" }));
	set("long", "他是干将的四弟子，专门铸造锤坯。\n" );
...
{
        message_vision("掌锤童子看了看$N说：你有事吗？\n",me);
}
...
int ask_me(object me)
{
	...
	{
		message_vision("掌锤童子不耐烦的对$N说：没看我正忙着呢么，什么事等会再说。\n", me);
		return 1;
	}
	message_vision("掌锤童子问$N：你带来了我师傅的信件么？没有师傅的允许，我不能为您打造。\n", me);
	return 1;
}
...
int accept_object(object me, object ob)
{
	if (this_object()->query_temp("item/铸造中"))
	{
		message_vision("掌锤童子不耐烦的对$N说：我正在忙，别打搅我。\n", me);
		return 0;
	}
	...
	switch (ob->query("id"))
	{
		...
		{
			message_vision("掌锤童子满脸疑惑的对$N说：你怎么有这么多师傅的信啊。\n", me);
		} else
		{
			me->set_temp("item/step", "接受原料");
			message_vision("掌锤童子对$N说：你把原料和配料给我，如果准备好了，咱们就开始(start)吧。\n", me);
		}
		return 1;
		...
		if (me->query_temp("item/step") != "接受原料")
		{
			message_vision("掌锤童子对$N说道：对不起，师傅教我们不要随便要别人的东西。\n", me);
			return 0;
		}
		// 接受铸锤原料
		if (ob->query("material_attrib") == "x")
		{
			...
			{
				message_vision("掌锤童子摇摇头说道：这个没法和" + 
					me->query_temp("item/material1") + "混在一起炼的。\n", me);
				return 0;
			}
			if (me->query_temp("item/material2"))
			{
				message_vision("掌锤童子说道：很好，原料配料都齐了，咱们是不是就开始啊。\n", me);
			} else
			{
				message_vision("掌锤童子看了看说道：很好，你有没有配料啊，要是没有。咱们就开始吧。\n", me);
			}
			...
			me->set_temp("item/item_weight", ob->query_weight() * 10);
			return 1;
		}
		// 接受铸锤配料
		if (ob->query("material_attrib") == "z")
		{
			if (me->query_temp("item/material2"))
			{
				message_vision("掌锤童子摇摇头说道：这只能做配料，你得给我原料啊。\n", me);
				return 0;
			}
			if (me->query_temp("item/material1"))
			{
				message_vision("掌锤童子说道：很好，原料配料都齐了，咱们是不是开始啊。\n", me);
			} else
			{
				message_vision("掌锤童子看了看说道：很好，你快点把原料给我吧。\n", me);
			}
		...
		message_vision("掌锤童子说：这东西也能练锤？我孤陋寡闻，你另请高就吧。\n", me);
		return 0;
	}
}
...
int	do_start()
{
	...
	switch (me->query_temp("item/step"))
	{
		...
		if (me->query_temp("item/material1"))
		{
			message_vision("掌锤童子说道：好，我开始铸锤了！\n", me);
			item_temp_make(me);
			break;
		}
		message_vision("掌锤童子眼睛一瞪，说道：开始什么，你让我拿什么练锤！？\n", me);
		break;

		default:
		message_vision("掌锤童子问$N：干什么？你是不是想请我喝酒？\n", me);
		break;
	}
	return 1;
}
...
int work_step_1(object me)
{
	...
	message_vision(HIY "\n掌锤童子把" + this_object()->query_temp("item/material1") + HIY "放进了火炉，" +
...
{
	message_vision(HIR "掌锤童子呼呼鼓动着风机，又添了一些精炭，火光越来越亮。\n\n" NOR, this_object());
...
int work_step_3(object me)
{
	...
	{
		message_vision(HIW "掌锤童子忙把" + this_object()->query_temp("item/material2") + HIW "添到铁水中，"
	...
	{
		message_vision(HIW "掌锤童子叹了口气道：可惜没有配料。\n\n" NOR, this_object());
	}
	message_vision(HIW "掌锤童子合好模具，在一旁小心的盯着。\n\n" NOR, this_object());
...
int work_step_4(object me)
{
	...
	{
		message_vision(HIW "掌锤童子小心翼翼的打开了模具，看了看，高兴地说：好了好了！\n\n" NOR, this_object());
		command("say 咦，人呢？怎么这就走了，东西都不要了。\n");
		this_object()->delete_temp("item");
		if (objectp(me)) me->delete_temp("item");
		return 1;
	}
	message_vision(HIW "掌锤童子小心翼翼的打开了模具，看了看，高兴地说：好了好了，快拿去给我师傅精炼吧。\n\n" NOR, me);
	// 生成粗坯
	ob = new("/d/item/obj/hammer_origin");
