...
object query_array_obstructor()
{
	...
	foreach(object ob in zhen->memb)
	{
		...
		if ((db["qi"] > db["max_qi"]*3/5)
		&& (db["jing"] > db["max_jing"]*3/5) )
			return ob;
	}
...
void check_array_status()
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (zhen->memb[i]->query_temp("is_riding"))
			break;

		if (zhen->memb[i]->query_skill_mapped("array") != ski) 
			break;
...
varargs void dismiss_team(object who)
{
	...
	else
	{
		...
		{
			message("team", sprintf(HIW "【队伍会话】%s脱离了队伍。\n" NOR,who->name(1)), oteam->memb);
...
object *query_team()
{
	...
	return classp(oteam) && oteam->memb;
}
