...
int perform_action(object me, string arg)
{
        ...
        if (target_ob)
        {
                avoid = me->query_temp("apply/avoid_noperform"); 
                nopfm = target_ob->query_temp("apply/target_noperform"); 
                if (nopfm > 80) nopfm = 80;
                if (me != target_ob && random(100) >= avoid && random(100) < nopfm)
                        return notify_fail(HIC BWHT + target_ob->name() + 
                                "全身突然发出一道耀眼的光芒，让你的绝招顿时无法发出。\n" NOR);
        }

        if (base_name(ob) != SKILL_D("six-finger") &&
            base_name(ob) != SKILL_D("dragon-strike") && 
            base_name(ob) != SKILL_D("dugu-jiujian")) {
                if (target_ob &&
                        ...
                        if ((int)call_other(file, "perform", me, me)) {
                                if (target_ob->query_skill_mapped("parry") == "qiankun-danuoyi")
                                        tell_object(all_inventory(environment(me)),
                                                sprintf(HIG + target_ob->query("name")+
                                                        "施展出绝技—— 「" NOR + BLINK + HIR +
                                                        "乾坤大挪移" NOR + HIG "」\n" +
