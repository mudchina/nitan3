...
int perform(object me)
{
    ...
    if( !victim || !victim->is_character() || !me->is_fighting(victim) )
        ...
        // array(打狗阵), third one is total staff skill of all people who join the 
        ...
        if( partner == 0 ) {
            ...
            skill += inarray[partner]->query_skill("staff");
            partner++;
...
if( partner = leader->query_temp("dagou/data")[2] ) {
                if(random(5) < 2) {
                        if( partner > 1 ) {
                                ...
                                // this is 棒掌结合。
                                if ( random(2) == 1 ) {                                       
                                        if( obj->query_skill_prepare()["strike"] == "dragon-strike" ) {
