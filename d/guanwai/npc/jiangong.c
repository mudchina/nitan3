...
mixed ask_mine()
{
       ...
       // object *obs;
        ...
        if (count_lt(me->query("combat_exp"), 50000))
                return "你这点经验也来采石场混？随便一大哥看你不顺眼就把你放到了。";

        if (count_gt(me->query("combat_exp"), 300000))
...
mixed ask_transit()
{
        ...
        if (count_lt(me->query("combat_exp"), 30000))
                return "一边儿去！你这人本事太差，我可不放心让你押货！";

        if (count_gt(me->query("combat_exp"), 1000000)) 
                return "你的经验这么高，我怎么好意思让你再运货呢！"; 
