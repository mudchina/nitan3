...
int show_bunch_info(object me, string arg)
{
        ...
        */
        ...
        msg += "帮派资产：" + count_div(BUNCH_D->query_bunch_info(fam, "money"), 10000) + " 两黄金\n\n";
#else
        msg += "帮派资产：" + MONEY_D->money_str(BUNCH_D->query(fam + "/money")) + "\n\n";
#endif
        msg += HIY "───────────────────────────\n" NOR;
        ...
        else 
        {
                ...
                if (arg == "with")
                {
                        ...
                        {                               
                                ...
                                tell_object(ob, bname + "的" + me->query("name") + "要求和贵帮派建立同盟关系。\n");
...
int bunch_arrest(object me, string arg)
{
        ...
        if (sscanf(arg,"-a %s", str) == 1)  
        {
                ...
                if (banghui == (string)target->query("bunch/bunch_name"))
                        message("channel:rumor",
                                ...
                                target->query("name")+"("+target->query("id")+")，帮中兄弟见到格杀勿论！\n"NOR, users());
                else
                        message("channel:rumor",
                                HIR"【谣言四起】"+ "某人：帮派「"+banghui+"」开始追杀"+target->query("name")+"("+target->query("id")+")，帮中兄弟见到格杀勿论！\n"NOR,users());
        ...
        if (sscanf(arg, "-d %s", str) == 1)  
        {
                ...
                BUNCH_D->add_bunch_info(banghui, "vendetta", vendetta);
                message("channel:rumor",
                       HIR "【谣言四起】"+ "某人：帮派「"+banghui+"」停止追杀"+target->query("name")+"("+target->query("id")+")，给予特赦。\n"NOR,users());
                UPDATE_D->global_destruct_player(target, 1);
                return 1;
        }
...
int bunch_invite(object me, string arg)
{
        ...
        {
                tell_object(ob, "如果你愿意加入对方的帮会，请使用 bunch join " + me->query("id") + "。\n");
                ob->set_temp("invite/target", me);
                return 1;
        }
}
