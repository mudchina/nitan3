...
mapping family = ([
        ...
        "华山派"   : ({ "/d/huashan/buwei1","/d/huashan/qunxianguan",
                       "/d/huashan/songlin1", }),
        "大轮寺"   : ({ "/d/xueshan/hubian1","/d/xueshan/chang",}),
...
void be_killed(int num)
{
        ...
        if (all_enemy <= 0)
                message("group_quest",HIR "【江湖情报】" NOR + HIY + "太棒了！所有的敌人全部都给消灭了！\n" + NOR,listeners);
        else if (all_enemy < 6)
                message("group_quest",HIR "【江湖情报】" NOR + HIY + "加油啊！根据最新情报，还剩下" + chinese_number(all_enemy) + "个敌人！\n" + NOR,listeners);
        return;
}
...
void init_npc_skill(object ob, int lvl)
{
        ...
        ob->add_temp("apply/attack", sk_lvl/4);
        ob->add_temp("apply/dodge", sk_lvl/8);
        ob->add_temp("apply/parry", sk_lvl/8);
        ob->add_temp("apply/damage", sk_lvl/2);
        ob->add_temp("apply/armor", sk_lvl/2);
        if (! ob->query_skills())
                return;
        ks = keys(ob->query_skills());
        for (i = 0; i < sizeof(ks); i++)
                ob->set_skill(ks[i], sk_lvl);
        ob->set("jiali", (int)ob->query_skill("force")/2);
}
...
int filter_listener(object ob) 
{
        if (! ob->query("channels")) return 0; 
        if (member_array("task", ob->query("channels")) == -1) return 0; 
        return 1; 
}
