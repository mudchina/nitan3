...
int perform(object me, object target)
{
	...
	object weapon;
        ...
        if (userp(me) && ! me->query("can_perform/yunv-jian/he"))
                return notify_fail("你未得高人指点，不知该如何施展「双剑合壁」。\n");
	...
	if( target->query_skill_mapped("sword") != "quanzhen-jian" )
		return notify_fail("你想与其双剑合璧的对象所使用的并非全真剑法！\n");
	...
	if( (int)target->query_temp("yunv_hebi") ) 
	...
	if (mapp(me->query("couple")) &&
	    me->query("couple/couple_id") == target->query("id"))
	skill += skill/2;
