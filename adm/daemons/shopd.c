...
public string do_list_all(object me, string arg);                       // 查询所有店铺 
public string check_shop_status();                                      // 查询所有店铺营业状况
...
public int is_inited()
{
        ...
        {
                shop = get_object(SHOP_DIR + all_shop[i]["start_room"]);
...
public string do_modify(object obj, object me, string arg)
{
        ...
        case "desc":
                ...
                obj->set("long", sort_msg(msg + NOR "\n"));
                room->set("waiter/long", sort_msg(msg + NOR "\n"));
...
public string do_stock(object ob, object me, string arg)
{
       ...
       object goods, obj2;
        ...
        if (goods->query("no_sell") || goods->query("id") == "key")
                return "这个东西太招摇了，还是别拿出来贩卖。\n";

        if (goods->query("task_ob"))
                return "这个东西太招摇了，还是别拿出来贩卖。\n";

        if (goods->is_item_make() || goods->query("unique"))
                return "这个东西太招摇了，还是别拿出来贩卖。\n";

        if (sscanf(base_name(goods), "/data/%*s"))
                return "这个东西太招摇了，还是别拿出来贩卖。\n";
        ...
        if (room->query("all_vendor_goods") >= 200) 
...
private int player_pay(object who, object target, int amount)
{
        ...
        if (v < amount) 
                ...
                */
                ...
                environment(target)->add("check_shop_profit", pay_amount);
                environment(target)->add("check_shop_tax", amount - pay_amount);
...
public string do_list_all(object me,string arg)
{
        ...
        for (j = 0; j < sizeof(all_shop); j++)
        {
                ...
                if (ob->query("ban") &&
                    member_array(me->query("id"), ob->query("ban")) != -1)
                {
                        // destruct(ob);
                        continue;
                }
        ...
        msg += HIC "≡" HIY "──────────────────────────────────" HIC "≡\n" NOR;
        all_user = filter_array(users(), (: $1->query_temp("on_bantan") :) );
        if (sizeof(all_user))
        {
...
public string check_shop_status()
{
        ...
        for (i = 0;i < sizeof(all_shop);i++)
        {
                ...
                profit = the_shop->query("check_shop_profit") / 10000 * 10000;
                all_profit += profit / 10000;
                tax = the_shop->query("check_shop_tax") / 10000 * 10000;
                all_tax += tax / 10000;
