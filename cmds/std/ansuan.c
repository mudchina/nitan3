...
int main(object me, string arg)
{
	...
	COMBAT_D->do_attack(me, target, me->query_temp("weapon") );
        ...
        if (count_gt(random(atoi(count_div(me->query("combat_exp"), 10))), count_div(target->query("combat_exp"), 20)) &&
            ! target->is_guarder())
	{
	...
	me->add_temp("str", - count);
        ...
        if (random(atoi(count_div(me->query("combat_exp"), 100))) < atoi(count_div(target->query("combat_exp"), 500)) ||
            target->is_guarder())
	{
