...
杂货铺等。一些居民从这里进进出出。你似乎觉得这里有些诡秘。旁边有个小木
门（door）。
LONG );
...
int do_knock(string arg)
{
        ...
        if (!arg||arg=="") 
          ...
          if( arg=="door" ) {
                ...
                if (me->query("bunch/bunch_name")!="黑白道" && (int)me->query("shen") <= 0)
                {
...
说完一把把$N拉了进去。\n"NOR, me);
                ...
                if( good_bunch(me) || (int)me->query("shen") > 0 && me->query("bunch/bunch_name")!="黑白道" ) 
                { 
