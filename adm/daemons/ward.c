...
void init_general(object me)
{
        ...
        if (party == "meng")
        {
                ...
                if (degree > 10) me->set("title", HIR "蒙古万夫长" NOR);
                else if (degree > 8) me->set("title", HIG "蒙古军那颜" NOR);
                else if (degree > 5) me->set("title", HIY "蒙古千夫长" NOR);
                else me->set("title", HIC "蒙古百夫长" NOR);
...
int marshal_die()
{
        message_ward(CHINESE_D->chinese_date((time()-14*365*24*60*60)) +
                MAG "，蒙古骑兵大败宋军于襄阳！！\n" +
                "              宋军主帅 " HIR + song_info->marshal->query("name") + " ( " + song_info->marshal->query("id") + " ) " NOR +
                MAG "战死沙场，以身殉国！！\n              靖康耻犹未雪，臣子恨何时灭啊！\n" NOR);
...
int start_kingwar(object me)
{
        ...
        for (i = 0; i < 4; i++)
        {
                ...
                general->set_temp("warquest/quest", "守卫蒙营，保护可汗");
                init_general(general);
                general = new("/d/city2/meng/mengj");
        ...
        for (i = 0; i < 2; i++)
        {
                ...
                general->set_temp("warquest/quest", "守卫草原，阻止入侵");
                init_general(general);
        }
        ...
        for (i = 0; i < 2; i++)
        {
                ...
                general->set_temp("warquest/quest", "守卫蒙营，阻止入侵");
                init_general(general);
        }
        ...
        jianjun->set_temp("warquest/quest", "留守宋营，监察军事");
...
string join_kingwar(object me)
{
        ...
        if ((time() - start_time) > 1800)
                return "现在前方战事快要结束了，将军以后再来吧！\n";

        /*
        if (member_array(query_ip_number(me), ip_numbers) != -1)
                return "每个IP上只可一个ID参军，将军还是以后再来吧！\n";
        */
        
        if (me->query("age") < 16)
                return "你还小，以后再来为国效力吧！\n";

        if (count_lt(me->query("combat_exp"), 100000))
                return "其志可嘉，但无奈手无缚鸡之力，你还是请回吧！\n";
...
void check_assignment()
{
        ...
        if (sizeof(total) < 1)
        {
                ...
                message("channel:rumor",
                        MAG "【火线军情】大宋：蒙古突骑兵潜进襄阳城内烧杀抢掠，急待增加守城兵力！！！\n" NOR, users());
                ...
                for (i = 0; i < sizeof(total); i++)
                {
                        ...
                        if (file == "/d/city2/syuanmen1")
                        {
                                if (room->query("defence") > 0 && ! ob->query_temp("warquest/break"))
                                        message("channel:rumor", MAG "【火线军情】大宋：一名" +
                                                ob->query("title") + MAG "率领的蒙古突骑兵已经攻到宋军大营，" +
                                                "宋军吃紧，急待增援！！！\n" NOR, users());
                                ...
                                if (ob->query_temp("warquest/break")) {
                                        message("channel:rumor", MAG "【火线军情】大宋：宋军大营" +
                                                MAG "被蒙古骑兵攻破，形势危急！！！\n" NOR, users());
                        ...
                        if (ob->query_temp("warquest/move") == "forward")
                        {
                                ...
                                if (file == "/d/xiangyang/guangchang")
                                {
                                        if (ob->query_temp("warquest/purchase") != "forage")
                                        {
                                                ...
                                                {
                                                case "weapon" :
                                                        message_vision(HIY "$N" HIY "率领的大军到达了襄阳城，命令部下开始购置兵器和盔甲，\n" +
                                                                "$N" HIY "将前线所需兵甲载上马车，又立刻开始了行军的奔程....\n" NOR, ob);
                                                        break;
                                                case "horse"  :
                                                        message_vision(HIY "$N" HIY "率领的大军到达了襄阳城，命令部下开始购置战马，\n" +
                                                                "$N" HIY "将前线所需战马全部带上，又立刻开始了行军的奔程....\n" NOR, ob);
                                                        break;
                                                case "stone"  :
                                                        message_vision(HIY "$N" HIY "率领的大军到达了襄阳城，命令部下开始购置石头和檑木，\n" +
                                                                "$N" HIY "将前线所需石木载上马车，又立刻开始了行军的奔程....\n" NOR, ob);
                                                        break;
                                                case "arrow"  :
                                                        message_vision(HIY "$N" HIY "率领的大军到达了襄阳城，命令部下开始购置羽箭，\n" +
                                                                "$N" HIY "将前线所需羽箭载上马车，又立刻开始了行军的奔程....\n" NOR, ob);
                                                        break;
                                                case "enlist" :
                                                        message_vision(HIY "$N" HIY "率领的大军到达了襄阳城，命令部下开始招募新兵，\n" +
                                                                "$N" HIY "带上所招募的新兵，又立刻开始了行军的奔程....\n" NOR, ob);
                                                        break;
                                                default :
                                                        break;
                                                }
...
void auto_cost()
{
        ...
        {
                message("channel:rumor",
                        MAG "【火线军情】大宋：饷银短缺军队出现逃兵，士气下降！！\n" NOR, users());
                have1 = 1;
        }
        if (song_info->forages < 1 && ! have2)
        {
                message("channel:rumor",
                        MAG "【火线军情】大宋：粮草短缺军队出现逃兵，士气下降！！\n" NOR, users());
                have2 = 1;
        }
...
int check_out(object me)
{
        ...
        message("channel:rumor", MAG "【火线军情】大宋：" + msg + "\n" NOR, users());
...
{
        message("channel:rumor", MAG "【火线军情】大宋：" + MAG + me->name(1) +
                            "在战场上临阵脱逃。\n", users());
        restore_status(me);
...
int general_die(object me)
{
        ...
        message_ward(CHINESE_D->chinese_date((time()-14*365*24*60*60)) +
                MAG "，宋军将领 " HIR + me->query("name") + " ( " + me->query("id") + " ) " NOR +
                MAG "战死沙场，以身殉国！！\n" NOR);
...
void message_ward(string msg)
{
        ...
        message("channel:rumor", MAG "【大宋史记】" + msg + NOR, users());
}
        ...
        if (arg == "meng")
        {
                    ...
                    meng_info->soilders > 24000)
                {
                        for (int i = 0; i < 3; i++)
                        {
                                ...
                                general->move(room);
                                init_general(general);
                                general->set_temp("warquest/quest", "攻打宋营，入侵大宋");
                        }
                }
        } else
        if (arg == "song")
        ...
        */
        ...
        case "chuzheng":
                ...
                if (inv[i]->query_temp("warquest/group") > 0)
                {
                        ...
                        inv[i]->set_temp("warquest/quest", "军队出征，迎战蒙军");
                }
                return "众将大声应道：末将遵命！！\n";
                break;
        ...
        case "zhencha" :
                ...
                ob->set("party/rank", HIY "侦察营统领" NOR);
                ...
                ob->set_temp("warquest/quest", "深入蒙境，侦察敌情");
                // ob->command("recon");
                break;
        case "enlist":
        case "zhaomu-xinbing" :
        ...
        return ob->query("name") + "大声应道：末将遵命！！\n";
}
