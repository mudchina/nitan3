...
int do_xiulian(string arg)
{
        ...
        if( me->query("quanzhen/shentong_times") > 20 ) 
        {
                me->set("can_perform/xiantian-gong/shen", 1);
                return notify_fail("你修练已久，该去实战应用一下了。\n");
        }
...
HIG"$N只觉得真气通过任督，真气充沛，偏体舒畅无比。\n"NOR,me);
                        ...
                        {
                                me->set("potential",count_sub(me->query("potential"), 1));
...
int do_qiecuo(string arg)
{
        ...
        if( count_lt(count_mul(target->query("combat_exp"), 3), count_mul(me->query("combat_exp"), 2)) )
