...
void create()
{
        set("short", "擂台");
        set("long", @LONG
这是一个四丈见方的擂台。粗壮结实的木柱撑起一片平台，四角的支柱上高
挂着四副对联，四面的观众都能清楚地看到台上的手起脚落。梁上贴着一长条幅
(tiaofu)，上面写了一些东西。
LONG );
        set("outdoors", "city");
        set("item_desc", ([
...
int do_jiangli(string arg)
{
        ...
        message("vision", HIW "【武林盛会】恭喜" + ob->query("name") + "(" +
                ob->query("id") + ")获得本届比武大会 第" +
                chinese_number(place) + "名。\n" NOR,
                all_interactive());
        message_vision("$N在怀里掏了半天，摸出一" + obv->query("unit") +
                       obv->name() + "交给了$n！\n", me, ob);
