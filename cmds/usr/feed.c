...
int main(object me, string arg)
{
	...
	if (! arg)
        ...
        if (arg == "all" || arg == "ALL")
        {
                ...
                if (count_lt(me->query("potential"), count_add(me->query("learned_points"), 50)))
	...
	if (me == ob)
        ...
        if (count_gt(count_add(me->query("learned_points"), 10), me->query("potential")))
        ...
        if (! wizardp(ob) && (count_lt(random(lvl * lvl) + lvl * lvl, ob->query("combat_exp")) ||
            count_ge(ob->query("combat_exp"), 5000000)))
        {
