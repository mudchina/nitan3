...
void hitself(object me, object *bad_guys, int degree) 
{
	...
	killer = bad_guys[i];
 	if (killer == 0) continue;
        a0=killer->query("combat/PKS");
        a1=killer->querry("combat/dietimes");
        b0=me->query("combat/PKS");
...
object *find_bad_guys(object me, object room)
{
	...
	if (userp(killer) && living(killer) &&
	    killer->query("combat/PKS") > 10 &&
	    killer->query("combat/PKS") > killer->query("dietimes")*2 &&
	...
	if (userp(killer) && living(killer) &&
	    ...
	    killer->query("combat/PKS") > killer->query("dietimes")*2 &&
            killer->is_killing(me->query("id"))) {
	    ret[n] = killer;
            if (me->query("id")=="smallfish") tell_object(me,"Bad guy is "+killer->query("id")+". \n");
	    n++;
  	}
    }
    return ret;
