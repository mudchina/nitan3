...
// 恢复调整
// {
nosave int change_max = 25000000; /* silver */
...
nomask varargs protected void do_count(string name,int silver)
{
	...
	for(i=0;i<n;i++)
	{
		foreach(class ware oneitem in all_wares[kinds[i]])
		{
			...
			origin = count_add(origin, count_mul(oneitem->o_value, oneitem->o_amount)); /* silver数 */
			cha = count_add(cha, count_mul((oneitem->o_amount -  oneitem->amount), oneitem->o_value));
#else
...
#ifdef LONELY_IMPROVED
	rate = atoi(count_div(count_add(cha, (system_change*100)), count_div(origin, 100)));
#else
...
#ifdef LONELY_IMPROVED
	if(stringp(name) && silver && (abs(silver) >= atoi(count_div(origin, 100))))
#else
...
nomask protected void get_shougou_amount(string arg,object me,class ware w)
{
	...
	*/
#ifdef LONELY_IMPROVED
        if (count_lt(me->query("balance"), value*100))
#else
