...
void random_move()
{
            ...
            random(all_enemy) == all_enemy/2)
        {
                ...
                if (inv)
                        inv = filter_array(inv, (: !userp($1) && $1->query("race") == "人类" :));
                if (inv)
                        inv = filter_array(inv, (: strsrch(base_name($1),"/kungfu/class/generate/") == -1 :));
                if (inv)
                        inv = filter_array(inv, (: count_le($1->query("combat_exp"), 500000) :));
                if (inv)
...
void jiuming(object npc,object enemy)
{
        ...
        if (npc->query("gender") == "女性" && random(2))
	...
	listeners = filter_array(users(), (: filter_listener :)); 
        ...
        message("group_quest",HIR "【外敌入侵】" NOR + WHT + msg + NOR,listeners);
}
