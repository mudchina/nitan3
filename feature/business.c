...
mixed profit = 0;  	/* 共赚取的利润(silver) */
mixed query_profit() { return profit; }
...
nomask int sell_out(string id,int amount,int value)
{
	...
	for(i=0;i<n;i++)
	{
		...
		{
			profit = count_add(profit, (value - all[i]->buy)*amount);
...
nomask string query_business_status()
{
	...
	else if(count_lt(profit, 0))
		output += sprintf("你在从事贸易中共亏损了 %s 两白银。\n\n",count_add(profit, 0));
	else
		output += sprintf("你在从事贸易中共赚取了 %s 两白银。\n\n", count_add(profit, 0));
...
protected void get_fasong_where(string num,object me,string *ds,class business ware)
{
	...
	if(fs_fee < 20) // 1000 两白银。每次运费最少为 20 silver。
        ...
        if(count_lt(me->query("balance"), fs_fee*100))
#else
...
protected void random_happen(class business w)
{
	...
	tell_object(this_object(),sprintf(HIW "你的%s在运输途中%s了 %d %s。\n" NOR,
