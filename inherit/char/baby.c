...
mixed query_potential_limit()
{
	...
	p = count_div(query("combat_exp"), 5);
	p = count_add(p, count_mul(count_div(query_int(), 100), p));
...
void die()
{
        ...
        set("combat_exp", count_sub(query("combat_exp"), count_div(query("combat_exp"), 50)));

        if (count_gt(query("potential"), query("learned_points")))
     	add("potential", count_sub(query("learned_points"), count_div(query("potential"), 5)));
