...
void create()
{
        set_name(HIC"铁胎长弓"NOR,({ "tie bow","bow","gong" }) );
        set_weight(12000);
        if(clonep())
...
int do_shoot(string id, object me, object bow, object room, string what, int m)
{
    	...
    	if (objectp(ob = present(id, room)) && ob->is_character())
    	{
     		...
     		if (random(me->query_skill("arrow", 1)) > ob->query_skill("dodge", 1) / 4
                ||  random(atoi(count_div(me->query("combat_exp"), 100))) > atoi(count_div(ob->query("combat_exp"), 100)))
     		{
      			if (damage > 2000)
      			...
      			COMBAT_D->report_status(ob, random(2));  
      				tell_object(me, HIR"嘿嘿，目标应声而倒，真是好箭法！\n"NOR);
                        ...
                        ratio = count_div(me->query("combat_exp"), 100);
                        ratio = atoi(count_div(ob->query("combat_exp"), ratio)); 
