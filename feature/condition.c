...
nomask int affect_by(string cnd, mixed para)
{
        ...
        add = this_object()->query_temp("apply/add_poison");
        avoid = this_object()->query_temp("apply/avoid_poison");
        reduce = this_object()->query_temp("apply/reduce_poison");

        if (random(100) < avoid || this_object()->query("special_skill/dispel"))
        {
              return 0;
        }
